C
      SUBROUTINE PHSOLV(PR, HTR, TR, TMAX, TMIN, TRECMX, TRECMN, D1R,
     >                  DVR, DLR, I2PH, Q, IWANT, PROPR, IERR)
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C                     *** PHSOLV ***                                   C
C THIS ROUTINE FINDS THE REDUCED TEMPERATURE CORRESPONDING TO A GIVEN  C
C REDUCED PRESSURE PR AND REDUCED ENTHALPY HTR.  HTR IS ACTUALLY TR    C
C TIMES THE REDUCED ENTHALPY, SO YOU ARE SOLVING FOR H/(R*TC). THIS IS C
C BECAUSE H/RT MIGHT NOT BE MONOTONIC IN TEMPERATURE                   C
C                                                                      C
C ARGUMENT LIST:                                                       C
C  NAME  TYPE I/O? EXPLANATION                                         C
C ------ ---- ---- --------------------------------------------------  C
C PR      R    I   REDUCED (P/PC) PRESSURE                             C
C HTR     R    I   REDUCED ENTHALPY-TEMPERATURE PRODUCT (H/RT)/(TR)    C
...
C MAINTENANCE:                                                         C
C                                                                      C
C 20SEP95 - INITIAL CREATION BY AHH                                    C
C 22SEP95 - AHH: MAKE TMAX AND TMIN INPUT ARGUMENTS.  ALSO ADD ABS.    C
C           CONV. TOL. SINCE ENTHALPY COULD BE AT OR NEAR ZERO         C
C 28SEP95 - AHH: PARAMETERIZE NUMBER OF PROPERTIES                     C
C 02OCT95 - AHH: ENHANCE CHECKING OF BOUNDS                            C
...
C 17OCT03 - AHH: TIGHTEN CONVERGENCE TOLERANCES                        C
C                                                                      C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      PARAMETER (RTOL = 1.0D-10, ATOL=1.D-9)
      PBOT = PSMIN / PCW
      D1R = 0.D0
      DVR = 0.D0
      DLR = 0.D0
      ...
      TLOW = TMIN
C
C FIRST, EVALUATE HTR AT TMAX AND TMIN TO SEE IF VALUE IS WITHIN BOUNDS
...
C
        IF (DABS(HTR) .GT. ATOL) THEN
          IF (DABS(1.D0-HTNEW/HTR) .LT. RTOL) GO TO 999
        ENDIF
        IF(DABS(HTNEW-HTR) .LT. ATOL) GO TO 999
C
