...
__global__ void sanm2n_reset(const int& lk,const int& ng,const int& ng2, int& nxyz, float* hmesh, XS_PRECISION* xstf, XS_PRECISION* xsdf, float* eta1, float* eta2, float* m260, float* m251, float* m253, float* m262, float* m264, float* diagD, float* diagDI) {
...
__global__ void sanm2n_resetMatrix(const int& lk,const int& ng,const int& ng2, int& nxyz, double& reigv, XS_PRECISION* xstf, XS_PRECISION* xsnff, XS_PRECISION* xschif, XS_PRECISION* xssf, float* matMs, float* matMf, float* matM) {
    int lkg0 = lk * ng;
...
__global__ void sanm2n_prepareMatrix(const int& lk,const int& ng,const int& ng2, int& nxyz, float* m251, float* m253, float* diagD, float* diagDI, float* matM, float* matMI, float* tau, float* mu) {
    int lkd0 = lk * NDIRMAX;
...
__global__ void sanm2n_calculateTransverseLeakage(const int& lk,const int& ng,const int& ng2, int& nxyz, int* lktosfc, int* idirlr, int* neib, float* hmesh, float* albedo, float* jnet, float* trlcff0, float* trlcff1, float* trlcff2)
{
    int lkd0 = lk * NDIRMAX;
...
__global__ void sanm2n_calculateEven(const int& lk,const int& ng,const int& ng2, int& nxyz, float* m260, float* m262, float* m264, float* diagD, float* diagDI, float* matM, double* flux, float* trlcff0, float* trlcff2, float* dsncff2, float* dsncff4, float* dsncff6)
{
    int lkd0 = lk * NDIRMAX;
    ...
    for (size_t idir = 0; idir < NDIRMAX; idir++)
    {
        ...
        for (size_t igd = 0; igd < ng; igd++)
        {
            ...
            bt2[igd] = 2 * (matM(0, igd, lk) * flux(0, lk) + matM(1, igd, lk) * flux(0, lk) + trlcff0(igd, lkd));
