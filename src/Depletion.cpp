...
Depletion::Depletion(Geometry& g)
    : _g(g) {
}
...
void        Depletion::dep(const float& tsec, const XEType& xeopt, const SMType& smopt, const float& xeamp, const float* power) {
#pragma omp parallel for
    for (int l = 0; l < _g.nxyz(); ++l) {
        if (power[l] == 0) continue;
        dep(l, tsec, xeopt, smopt, xeamp, power[l], _dnst, _dnst_new, _dnst_avg);
    }
}

void Depletion::dep(const int& l, const float& tsec, const XEType& xeopt, const SMType& smopt, const float& xeamp, const float& power, float* ati, float* atd, float* atavg) {
    ...
    if (xeopt == XEType::XE_TR) {
        depxe(l, tsec, xeamp, ati, atd, atavg);
...
void Depletion::deph(const int& l, const float& tsec, const float* ati, float* atd, float* atavg) {
    ...
    // for (int ic = 0; ic < _nhvychn; ++ic) {
    //     for (int i = 0; i < nhchn(ic); ++i) {
    //         atd(ihchn(i, ic), l)   = ati(ihchn(i, ic), l);
    //         atavg(ihchn(i, ic), l) = ati(ihchn(i, ic), l);
    //     }
    // }
    // return;
}

void        Depletion::dynxesm(const float& tsec, const XEType& xeopt, const SMType& smopt, const float& xeamp, const double* xsmica, const double* xsmicf, const float* power, const double* flux, const float& fnorm) {
#pragma omp parallel for
    for (int l = 0; l < _g.nxyz(); ++l) {
        if (power[l] == 0) continue;
        ...
        if (xeopt == XEType::XE_TR) {
            ...
            depxe(l, tsec, xeamp, _dnst, _dnst_new, _dnst);
...
void Depletion::depxe(const int& l, const float& tsec, const float& xeamp, const float* ati, float* atd, float* atavg) {
    int   ipp  = I135;
    int   idd  = XE45;
    float dcp  = dcy(I135);
    ...
    for (int ih = 0; ih < NHEAVY; ++ih) {
        int ihf = ISOHVY[ih];
        fyp     = fyp + fis(ihf, l) * fyld(IFP_I135, ih) * xeamp * atavg(ihf, l);
...
void        Depletion::eqxe(const double* xsmica, const double* xsmicf, const double* flux, const float& fnorm) {
#pragma omp parallel for
    for (int l = 0; l < _g.nxyz(); l++) {
...
void Depletion::eqxe(const int& l, const double* xsmica, const double* xsmicf, const double* flux, const float& fnorm) {
    ...
    auto dnstI  = dnst(I135, l);
    auto dnstXE = dnst(XE45, l);

    dnst(I135, l) = fy_i135 / dcy(I135);
    dnst(XE45, l) = (fy_i135 + fy_xe45) / rem_xe;

    dnst(I135, l) = dnst(I135, l) * 0.5 + dnstI * 0.5;
    dnst(XE45, l) = dnst(XE45, l) * 0.5 + dnstXE * 0.5;
}
...
void Depletion::depsm(const int& l, const float& tsec, const float* ati, float* atd, float* atavg) {
    ...
    atd(SM49, l) = asm1 / rem(SM49, l) * (1. - expsm) + ati(SM49, l) * expsm;

    if (exp49 - expsm != 0.0) atd(SM49, l) += bsm / (rem(SM49, l) - rem(PM49, l)) * (exp49 - expsm);
    if (exp48s - expsm != 0.0) atd(SM49, l) += csm / (rem(SM49, l) - rem(PS48, l)) * (exp48s - expsm);
    if (exp48m - expsm != 0.0) atd(SM49, l) += dsm / (rem(SM49, l) - rem(PM48, l)) * (exp48m - expsm);
    if (exp47 - expsm != 0.0) atd(SM49, l) += esm / (rem(SM49, l) - rem(PM47, l)) * (exp47 - expsm);
...
void        Depletion::pickData(const double* xsmica, const double* xsmicf, const double* xsmic2n, const double* flux, const float& fnorm) {
#pragma omp parallel for
    for (int l = 0; l < _g.nxyz(); ++l) {
...
void Depletion::pickData(const int& l, const double* xsmica, const double* xsmicf, const double* xsmic2n, const double* flux, const float& fnorm) {
    ...
    for (int iiso = 0; iiso < NDEP; ++iiso) {
        ...
        for (int ig = 0; ig < _g.ng(); ++ig) {
            double xsa = xsmica(ig, iiso, l);
            double xsc = xsmica(ig, iiso, l) - xsmicf(ig, iiso, l);
            if (iiso == U238 && ig == 0) {
                xsa += 2 * xsmic2n(ig, l);
                xsc += xsmic2n(ig, l);
            }
