...
sanm2n_reset(const int &lk, const int& ng, const int& ng2, int &nxyz, float *hmesh, XS_VAR  *xstf, XS_VAR  *xsdf, float *eta1,
             float *eta2, float *m260, float *m251, float *m253, float *m262, float *m264, float *diagD,
             float *diagDI);

__host__ __device__ void
sanm2n_resetMatrix(const int &lk, const int& ng, const int& ng2, int &nxyz, double &reigv, XS_VAR  *xstf, XS_VAR  *xsnff,
                   XS_VAR  *xschif, XS_VAR  *xssf, float *matMs, float *matMf, float *matM);
...
__host__ __device__ void sanm2n_calculateJnet2n(const int &ls, const int& ng, const int& ng2, int &nsurf, int *lklr, int *idirlr, int *sgnlr, float *hmesh,
                                     XS_VAR  *xsadf, float *m260, float *m262, float *m264, float *diagD,
...
__host__ __device__ void sanm2n_calculateJnet1n(const int& ls, const int& lr, const int& ng, const int& ng2, int& nsurf, int* lklr, int* idirlr, int* sgnlr, float* hmesh, const float& albedo, XS_VAR * xsadf, float* m251, float* m253, float* diagD, float* matM, double* flux, float* trlcff1, float* eta1, float* eta2, float* dsncff2, float* dsncff4, float* dsncff6, float* jnet);
