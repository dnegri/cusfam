...
static map<int, shared_ptr<Snapshot>> simonSnapshots;
...
//                   const int* ijtol, const int* neibr, const double* hmesh, const double* vol, const double* albedo);

// void initsfamxsec(const int& ng, const int& ng2s, const int& ndim, const int& nxy, const int& nz,
//                   const double* xschif, const double* xsdf, const double* xstf, const double* xsnf,
//                   const double* xskf, const double* xssf);

// }

unique_ptr<SimonCPU> simon_init(const char* geom_file, const char* tset_file, const char* ff_file, int nthreads) {
    ...
    //              2, 25, g.nxs(), g.nxe(), g.nys(), g.nye(), g.ijtol(), g.neibr(), g.hmesh(), g.vol(), g.albedo());
...
void simon_setBurnup(SimonCPU& simon, const char* dir_burnup, float burnup, SteadyOption& option) {
    simon.setBurnup(dir_burnup, burnup, option);
    simon.updateBurnup();
}

void simon_saveSnapshot(int id, SimonCPU& simon) {
    auto shot = make_shared<Snapshot>(simon.g());
    simonSnapshots.insert({id, shot});
}

void simon_loadSnapshot(int id, SimonCPU& simon) {
    auto snapshot = simonSnapshots[id];
    snapshot->load(simon);
}


void simon_calcStatic(SimonCPU& simon, SteadyOption& option) {
    simon.runSteady(option);
}

void simon_searchASI(SimonCPU& simon, SteadyOption& option, float targetASI) {
    ASISearcher asis(simon.g());
    asis.search(1.E-4, 10, simon, option, targetASI, 0.001);
}

void simon_resetASI(SimonCPU& simon) {
    std::fill_n(simon.x().delmac(), simon.g().ng() * simon.g().nxyz(), 0.0);
}


void simon_setPowerShape(SimonCPU& simon, std::vector<double> hzshp, std::vector<double> powshp) {

    simon.setPowerShape(hzshp, powshp);
}
...
void simon_depleteByTime(SimonCPU& simon, XEType xe_option, SMType sm_option, float tsec, float xeamp = 1.0) {
...
PYBIND11_MODULE(cusfam, m) {
    ...
    py::class_<SteadyOption>(m, "SteadyOption")
            ...
            .def_readwrite("b10a", &SteadyOption::b10a)
    ...
    py::enum_<ShapeMatchOption>(m, "ShapeMatchOption")
            .value("NO",ShapeMatchOption::SHAPE_NO)
            .value("HOLD",ShapeMatchOption::SHAPE_HOLD)
            .value("MATCH",ShapeMatchOption::SHAPE_MATCH)
            .export_values();
    ...
    m.def("setPowerShape", &simon_setPowerShape, R"pbdoc(
        Set Power Shape.
    )pbdoc");
    ...
    m.def("searchASI", &simon_searchASI, R"pbdoc(
        Search ASI.
    )pbdoc");

    m.def("resetASI", &simon_resetASI, R"pbdoc(
        Remove delta macroxs of ASI search.
    )pbdoc");
    ...
    m.def("saveSnapshot", &simon_saveSnapshot, R"pbdoc(
        Save a snapshot of the current simon state.
    )pbdoc");

    m.def("loadSnaphot", &simon_loadSnapshot, R"pbdoc(
        Load a snapshot.
    )pbdoc");
