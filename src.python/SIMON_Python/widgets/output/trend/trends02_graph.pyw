...
class trend02Widget(QWidget):
    def __init__(self,fixedTime_flag,asi_flag, cbc_flag, power_flag):
        ...
        # if not self.ASI_FLAG:
        #     self.subplot_adjust_right += 0.035
        # if not self.CBC_FLAG:
        #     self.subplot_adjust_right += 0.035
        # if not self.ASI_FLAG or not self.CBC_FLAG:
        ...
        if self.Power_FLAG:
            ...
            self.graph01_AxisY_Label = "Peaking Factor"
        ...
        self.cursor01 = Cursor(self.graph01, horizOn=True, vertOn=True, useblit=True, color='r',  linewidth=0.5)
        self.annot1 = self.graph01.annotate("", xy=(0, 0), xytext=(-40, 40),
                                      textcoords='offset points', color='white', fontsize=20,
    ...
    def defineRodPosGraph(self):
        self.graph01.tick_params(axis='x', which='major', colors=self.gridColor, width=1)
        self.graph01.tick_params(axis='y', which='major', colors=self.gridColor, width=1)
        ...
        self.graph01.yaxis.set_major_locator(ticker.FixedLocator([1.0, 1.5, 2.0, 2.5, 3.0]))#, 231, 281, 331, 381]))
        ...
        self.graph01.set_ylabel(self.graph01_AxisY_Label,font='Segoe UI',fontsize=12,fontweight='bold',labelpad=3.0,loc='center')#, position="right")
    ...
    def defineCBC_Graph(self):
        ...
        self.graph03.tick_params(axis='x', which='major', colors=self.gridColor, width=1)
        ...
        self.graph03.tick_params(axis='y', which='major', colors=self.gridColor, width=1)#, length=0)
    ...
    def makePlotFormat(self):
        ...
        self.line_F_XY = self.graph01.plot(self.time, self.data_P, marker='o', alpha=0.5, color=self.color_Rod_P, label="Fxy", markersize=5)
        self.line_F_R  = self.graph01.plot(self.time, self.data05, 'o-', alpha=0.5, color=self.color_Rod05, label="Fr", markersize=5)
        self.line_F_Q  = self.graph01.plot(self.time, self.data04, 'o-', alpha=0.5, color=self.color_Rod04, label="Fq", markersize=5)
        ...
        if(self.ASI_FLAG==True):
            ...
            self.data_ASI_BAND_UP = []  # 0.27,  0.27,  0.27,  0.27,  0.27,   0.27  ,    0.6  ,  0.60,  0.60,  0.60 ]
            self.data_ASI_BAND_DOWN = []  # -0.27, -0.27, -0.27, -0.27, -0.27,  -0.27  ,   -0.6  , -0.60, -0.60, -0.60 ]
            self.line_ASI_BAND_UP = None
            self.line_ASI_BAND_DOWN = None

        if(self.CBC_FLAG==True):
            self.data_CBC = []
            self.line_CBC = self.graph03.plot(self.time, self.data_CBC, 'D-',linewidth=2, alpha=1.0, color=self.color_CBC, label="CBC", markersize=5)
            self.d_CBC = self.convt_CBC_POS(self.data_CBC)
            self.dot_CBC = self.graph01.plot(self.time, self.d_CBC, 'D', visible=False, alpha=1.0, color=self.color_CBC, picker=True, pickradius=5, label="dot_CBC", markersize=5)
        ...
        self.lns = self.line_F_XY + self.line_F_R + self.line_F_Q
        if(self.ASI_FLAG==True):
    ...
    def insert_ECP_DataSet(self, time_temp, data_P, data_5, data_4, dataOpt):#, data_ASI, data_CBC, data_power):
        tmp = dataOpt
        if(self.ASI_FLAG==True):
        ...
        self.line_F_XY[0].set_data(time, self.data_P)
        self.line_F_R[0].set_data(time, self.data05)
        self.line_F_Q[0].set_data(time, self.data04)
    ...
    def insertDataSet(self, time, data_P, data_5, data_4, dataOpt):#, data_ASI, data_CBC, data_power):
        ...
        self.line_F_XY[0].set_data(self.time, self.data_P)
        self.line_F_R[0].set_data(self.time, self.data05)
        self.line_F_Q[0].set_data(self.time, self.data04)
    ...
    def clickEvent(self,event):
        ...
        # If Unit Line was deactived, dot-click event also deactivated
            ...
            if(self.legend_Actvated_Flag[index]==True):
                ...
                self.annot1.xy = (x, y)
                text = "{:.1f} hour".format(x)
                self.annot1.set_text(text)
                self.annot1.set_visible(True)
                # self.canvas.draw()
                self.fig.canvas.draw()

                return float(event.mouseevent.xdata)
                #pass
    ...
    def resizeMaxTimeAxes(self, max_time):
        self.max_Time = max_time
        divs = math.ceil((max_time-self.min_Time)/5.0)
