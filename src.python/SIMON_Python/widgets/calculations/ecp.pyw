...
import widgets.utils.PyRodPositionBarChart as AxialWidget
...
class ECPWidget(CalculationWidget):
    ...
    def __init__(self, db, ui, calManager, queue, message_ui):
        ...
        layA = QtWidgets.QVBoxLayout(self.ui.ECP_widgetChart)
        layA.setContentsMargins(0, 0, 0, 0)
        
        self.axialWidget = AxialWidget.AxialWidget()
        layA.addWidget(self.axialWidget)
        barWidgetWidth = self.axialWidget.width()
        barWidgetHeight = self.axialWidget.height()
        self.axialWidget.setChartSize(barWidgetWidth,barWidgetHeight)
        ...
        sizePolicy5.setVerticalStretch(10)
        sizePolicy5.setHeightForWidth(self.ui.ECP_widgetChart_2.sizePolicy().hasHeightForWidth())
        self.ui.ECP_widgetChart_2.setSizePolicy(sizePolicy5)

        lay = QtWidgets.QVBoxLayout(self.ui.ECP_widgetChart_2)
    ...
    def settingLinkAction(self):
        ...
        self.ui.ECP_Input07.valueChanged['double'].connect(self.BeforeRodPosChangedEvent)
        self.ui.ECP_Input08.valueChanged['double'].connect(self.BeforeRodPosChangedEvent)
        self.ui.ECP_Input09.valueChanged['double'].connect(self.BeforeRodPosChangedEvent)

        self.ui.ECP_Input14.valueChanged['double'].connect(self.AfterRodPosChangedEvent)
        self.ui.ECP_Input15.valueChanged['double'].connect(self.AfterRodPosChangedEvent)
        self.ui.ECP_Input16.valueChanged['double'].connect(self.AfterRodPosChangedEvent)
    ...
    def settingTargetOpt(self):
        ...
        if (self.ui.ECP_CalcTarget01.isChecked()):
            ...
            self.BeforeRodPosChangedEvent()
            self.axialWidget.RedrawBar02(381.0,381.0,381.0)

        if self.current_calculation:
            current_input = self.get_input(self.current_calculation)
            current_input.search_type = search_type

    def BeforeRodPosChangedEvent(self):

        self.posBeforeBankP = self.ui.ECP_Input07.value()
        self.posBeforeBank5 = self.ui.ECP_Input08.value()
        self.posBeforeBank4 = self.ui.ECP_Input09.value()
        self.axialWidget.RedrawBar01(self.posBeforeBankP,self.posBeforeBank5,self.posBeforeBank4)

    def AfterRodPosChangedEvent(self):

        self.posAfterBankP = self.ui.ECP_Input14.value()
        self.posAfterBank5 = self.ui.ECP_Input15.value()
        self.posAfterBank4 = self.ui.ECP_Input16.value()
        self.axialWidget.RedrawBar02(self.posAfterBankP,self.posAfterBank5,self.posAfterBank4)
    ...
    def show_output_values(self, show_index=0):
        ...
        self.ui.labelECP005.setText("{:.2f}".format(self.calcManager.out_ppm_old[show_index]))
        self.ui.labelECP006.setText("{:.5f}".format(self.eigen))
        ...
        if self.ECP_TargetOpt==df.select_RodPos:
            ...
            self.ui.labelECP014.setText("{:.2f}".format(self.calcManager.rodPos[show_index][1]))
            self.ui.labelECP015.setText("{:.2f}".format(self.calcManager.rodPos[show_index][2]))
            self.axialWidget.RedrawBar02(self.calcManager.rodPos[0][0],self.calcManager.rodPos[0][1],self.calcManager.rodPos[0][2])
