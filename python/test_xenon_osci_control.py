...
if __name__ == "__main__":
    ...
    endTime += 12*3600. # cool time with 80% power
    ...
    time = pre.runPowerUp(powerRatio, ppmRatio*1/2, s, stdopt, targetASI, endTime, delTime, time, result)

    # endTime += powerChange / powerRatio
    # time = pre.runPowerUp(0.0, ppmRatio*1/2, s, stdopt, targetASI, endTime, delTime, time, result)
    ...
    # xenon osci. control
    endTime += 200.*3600.
    endTime = 14 * 24 * 3600.0

    critASI = 0.002;
    while time <= endTime:
        time += delTime
        s.depleteByTime(XE_TR, SM_TR, delTime, 0.98)
        s.calculateStatic(stdopt)
        
        if result.asi < targetASI - critASI : 
            s.searchRodPositionDown(stdopt, targetASI, result)
        elif result.asi > targetASI + critASI : 
